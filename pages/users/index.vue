        ...
        >
          <div class="flex items-center gap-4">
            <span class="text-sm font-medium text-slate-600">
              Total:
              <span class="font-bold text-orion-primary">{{
                enrichedUsersList.length
              }}</span>
              clients
            </span>
            ...
            <tr v-if="filteredClients.length === 0" class="hover:bg-slate-50">
              <td colspan="7" class="px-6 py-8 text-center text-slate-400">
                <div v-if="users.length === 0" class="space-y-2">
                  ...
                  <p class="text-xs">
                    Les données seront chargées depuis l'API /user/
                  </p>
                </div>
...
// Enrichir les clients avec les statuts
const enrichedUsersList = computed(() => {
  return users.value.map((client) => ({
    ...client,
...
const filteredClients = computed(() => {
  let result = enrichedUsersList.value;
