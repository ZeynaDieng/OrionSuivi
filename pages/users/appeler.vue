  ...
  <div class="p-4 md:p-8 lg:p-10 max-w-[1600px] mx-auto">
    ...
    <header class="mb-10">
      ...
      <p class="text-slate-500 font-medium">
        Clients expirés nécessitant un appel urgent
      </p>
    </header>
          ...
          <tbody class="divide-y divide-slate-100 bg-white">
            <tr v-if="filteredClients.length === 0" class="hover:bg-slate-50">
              <td colspan="7" class="px-6 py-8 text-center text-slate-400">
                <div v-if="expiredClientsList.length === 0" class="space-y-2">
                  <p class="font-medium">Aucun client à appeler</p>
                  <p class="text-xs">
                    Tous les clients sont à jour ou actifs
                  </p>
                </div>
            ...
            >
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center gap-3">
                  <div
                    class="h-10 w-10 rounded-full bg-red-500 flex items-center justify-center text-white text-xs font-bold"
                  >
...
const expiredClientsList = computed(() => {
  return users.value
    .map((client) => ({
      ...client,
    ...
    .filter((client) => client.status === "expire");
});

// Filtrer les clients selon la recherche et les dates
const filteredClients = computed(() => {
  let result = expiredClientsList.value;
...
// Réinitialiser la page quand la recherche ou les dates changent
watch(
  [searchQuery, dateStart, dateEnd, inscriptionDateStart, inscriptionDateEnd],
  () => {
    currentPage.value = 1;
  }
);
